    ...
    <div class="relative flex flex-col gap-[30px] my-[60px] px-5">
        ...
        <div id="Header" class="relative flex items-center justify-between gap-2">
            <div class="flex flex-col w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white">
                    ...
                    <div class="flex flex-col w-full gap-3">
                        ...
                        <div class="flex items-center gap-[6px]">
                            <img src="{{ asset('assets/images/icons/location.svg') }}" class="flex w-5 h-5 shrink-0"
                                alt="icon">
                            <p class="text-sm text-ngekos-grey">Kota
                                {{ optional(optional($boardingHouse->boardingHouse)->city)->name ?? 'Unknown' }}</p>
                        </div>
                        <div class="flex items-center gap-[6px]">
                ...
                <div class="flex gap-4">
                    ...
                    <div class="flex flex-col w-full gap-3">
                        <p class="font-semibold text-lg leading-[27px]">{{ $boardingHouse->name }}</p>
                        <hr class="border-[#F1F2F6]">
                        <div class="flex items-center gap-[6px]">
                            <img src="{{ asset('assets/images/icons/profile-2user.svg') }}" class="flex w-5 h-5 shrink-0"
                                alt="icon">
                        ...
                        <div class="flex items-center gap-[6px]">
                            <img src="{{ asset('assets/images/icons/3dcube.svg') }}" class="flex w-5 h-5 shrink-0"
                                alt="icon">
                            <p class="text-sm text-ngekos-grey">{{ $boardingHouse->square_feet }}sqft flat</p>
                        </div>
                        <div class="flex items-center gap-[6px]">
        ...
        <div class="flex flex-col gap-[18px]">
            ...
            <div class="flex items-center rounded-full p-[14px_20px] gap-3 bg-[#F5F6F8]">
                <img src="{{ asset('assets/images/icons/note-favorite-green.svg') }}" class="flex w-5 h-5 shrink-0"
                    alt="icon">
        ...
        <div class="flex flex-col gap-[14px]">
            ...
            <form action="{{ route('check-booking.show') }}" method="POST">
                ...
                <button class="w-full rounded-full p-[14px_20px] text-center font-bold text-white bg-ngekos-orange">View
                    My Booking</button>
            </form>
        </div>
    @endsection
